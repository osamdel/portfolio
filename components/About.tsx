...
import { ArrowUpRight, Check, Cpu, Server, Globe, Activity, Wifi, Lock, Unlock, Layers, GitBranch, Fingerprint, Scan, Database, Smartphone, Terminal, Signal } from 'lucide-react';
       ...
       <div className="relative z-10 p-6 h-full flex flex-col justify-between">
          <div className="flex justify-between items-start">
             ...
             <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-cyan-950/50 border border-cyan-800/50">
                ...
                <span className="text-[10px] font-mono font-bold text-cyan-300">{t.about.activeStatus}</span>
             </div>
          </div>

          <div>
             <div className="flex items-baseline gap-2 mb-1">
                <h3 className="text-3xl font-bold text-white tracking-tight">{t.about.locationMain}</h3>
                <span className="text-sm font-medium text-gray-500">{t.about.locationSub}</span>
             </div>
             
             <div className="space-y-2 mt-4">
                <div className="flex justify-between text-xs font-mono text-gray-400 border-b border-white/5 pb-1">
                   <span>{t.about.netLabel}</span>
                   <span className="text-cyan-200">{t.about.netValue}</span>
                </div>
                <div className="flex justify-between text-xs font-mono text-gray-400 border-b border-white/5 pb-1">
                   <span>{t.about.accessLabel}</span>
                   <span className="text-cyan-200">{t.about.accessValue}</span>
                </div>
                <div className="flex justify-between text-xs font-mono text-gray-400">
                   <span>{t.about.syncLabel}</span>
                   <span className="text-white">{t.about.syncValue}</span>
                </div>
             </div>
          </div>
       </div>
...
const ConnectCard = () => {
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    toast.success("Signal Verified: Email Copied");
    ...
    >
      ...
      <div className="flex justify-between items-start relative z-10">
        <div className={`p-2 rounded-xl backdrop-blur-md border transition-colors duration-300 ${copied ? 'bg-green-500/20 border-green-500/50' : 'bg-white/5 border-white/10'}`}>
          {copied ? <Check className="w-5 h-5 text-green-400" /> : <Signal className={`w-5 h-5 ${hovered ? 'text-indigo-400 animate-pulse' : 'text-gray-400'}`} />}
        </div>
  ...
  return (
    <section id="about" className="py-20 md:py-32 bg-gray-950 scroll-mt-0 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {/* Profile Card - Tall Left (2x2) */}
          <motion.div variants={itemVars} className="md:col-span-2 md:row-span-2">
            <TiltCard>
              <ProfileCard />
            </TiltCard>
          </motion.div>

          {/* Presence - Top Right (1x1) */}
          ...
          <motion.div variants={itemVars} className="md:col-span-1 md:row-span-1">
           ...
           {/* Tech Scanner - Tall Bottom Right (2x2) */}
           <motion.div variants={itemVars} className="md:col-span-2 md:row-span-2">
            <TiltCard>
              <TechScannerCard />
            </TiltCard>
