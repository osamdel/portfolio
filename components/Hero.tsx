...
const FloatingBadge = ({ children, className, delay = 0, show }: { children: React.ReactNode, className?: string, delay?: number, show: boolean }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20, scale: 0.8 }}
      animate={show ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 20, scale: 0.8 }}
      transition={{ duration: 0.8, delay }}
      className={cn("absolute z-30 flex items-center justify-center", className)}
    >
      <motion.div
        ...
        className="backdrop-blur-xl bg-white/80 dark:bg-gray-900/80 border border-white/20 dark:border-white/10 shadow-2xl rounded-2xl p-2.5 md:p-4 flex items-center gap-2 md:gap-3"
      >
        {children}
      </motion.div>
    </motion.div>
    ...
    <section id="hero" className="relative min-h-[100vh] flex flex-col items-center justify-center overflow-hidden bg-gray-50 dark:bg-gray-950 pt-28 pb-20 transition-colors duration-300">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full flex flex-col items-center">
        ...
        <div className="relative w-full flex flex-col items-center">
          ...
          {/* IMAGE CONTAINER */}
          {/* Adjusted to be a fixed square aspect ratio for all devices */}
          <div className="relative w-full max-w-[340px] sm:max-w-[400px] md:max-w-[500px] aspect-square mx-auto flex items-center justify-center mb-10 sm:mb-12">
             
             {/* 1. Floating Elements (The "Pro" Touch) */}
             
             {/* Left: Code / Tech */}
             <FloatingBadge className="-left-4 md:-left-[10%] top-[15%] md:top-[20%]" delay={0.2} show={startAnimation}>
                <div className="p-1.5 md:p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg text-blue-600 dark:text-blue-400">
                  <Code2 className="w-4 h-4 md:w-5 md:h-5" />
                </div>
                <div className="flex flex-col">
                   <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t.hero.badges.code}</span>
                   <span className="text-xs md:text-sm font-bold text-gray-900 dark:text-white">{t.hero.badges.architecture}</span>
                </div>
             </FloatingBadge>

             {/* Right: Design / Visuals */}
             {/* Mobile: Bottom Left (aligned with Score). Desktop: Top Right */}
             <FloatingBadge 
                className="-left-4 bottom-[10%] md:left-auto md:-right-[10%] md:top-[30%] md:bottom-auto" 
                delay={0.4} 
                show={startAnimation}
             >
                <div className="p-1.5 md:p-2 bg-purple-100 dark:bg-purple-900/30 rounded-lg text-purple-600 dark:text-purple-400">
                  <Sparkles className="w-4 h-4 md:w-5 md:h-5" />
                </div>
                <div className="flex flex-col">
                   <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t.hero.badges.design}</span>
                   <span className="text-xs md:text-sm font-bold text-gray-900 dark:text-white">{t.hero.badges.visuals}</span>
                </div>
             </FloatingBadge>

             {/* Bottom Right: Performance */}
             <FloatingBadge className="-right-2 md:-right-[5%] bottom-[10%] md:bottom-[15%]" delay={0.6} show={startAnimation}>
                <div className="p-1.5 md:p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg text-emerald-600 dark:text-emerald-400">
                  <Zap className="w-4 h-4 md:w-5 md:h-5" />
                </div>
                <div className="flex flex-col">
                   <span className="text-xs md:text-sm font-bold text-gray-900 dark:text-white">{t.hero.badges.score}</span>
                </div>
             </FloatingBadge>

             {/* 2. Main Photo - Square Frame */}
             <motion.div
               initial={{ opacity: 0, scale: 0.9, y: 20 }}
               animate={startAnimation ? { opacity: 1, scale: 1, y: 0 } : { opacity: 0, scale: 0.9, y: 20 }}
