...
import { ArrowUpRight, Code2, Send } from 'lucide-react';
...
export const Footer: React.FC = () => {
  ...
  const socialLinks = [
    { name: 'LinkedIn', href: 'https://www.linkedin.com/in/osamdel/' },
    { name: 'Instagram', href: 'https://www.instagram.com/osamdel/' },
  ];
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-32 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 mb-16 md:mb-24 border-t border-white/10 pt-12 md:pt-20">
          ...
          <div className="md:col-span-5 flex flex-col justify-between gap-8">
             <div>
               ...
               <p className="text-lg md:text-xl text-gray-400 max-w-sm leading-relaxed font-medium">
                 Crafting digital experiences that merge engineering precision with aesthetic excellence.
               </p>
             </div>
             <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full backdrop-blur-sm">
                   ...
                   <span className="text-sm font-bold text-gray-300">Istanbul, Turkey</span>
                </div>
                <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-full backdrop-blur-sm hidden sm:block">
                   <span className="text-sm font-bold text-gray-300">Local Time: {new Date().toLocaleTimeString('en-US', { timeZone: 'Europe/Istanbul', hour: '2-digit', minute: '2-digit', hour12: false })}</span>
                </div>
             </div>
          </div>
          ...
          <div className="md:col-span-3 md:col-start-7">
            <h4 className="text-sm font-bold uppercase tracking-widest text-gray-500 mb-6 md:mb-8">Sitemap</h4>
            <ul className="space-y-4">
              {['About', 'Work', 'Services', 'Contact'].map((item) => (
                <li key={item}>
                  <a 
                    href={`#${item.toLowerCase() === 'about' ? 'hero' : item.toLowerCase()}`}
                    onClick={(e) => handleNavClick(e, `#${item.toLowerCase() === 'about' ? 'hero' : item.toLowerCase()}`)} 
          ...
          <div className="md:col-span-3">
             <h4 className="text-sm font-bold uppercase tracking-widest text-gray-500 mb-6 md:mb-8">Socials</h4>
             <ul className="space-y-4">
               {socialLinks.map((social) => (
                 <li key={social.name}>
                   <a 
                     ...
                     className="group flex items-center justify-between text-lg font-medium text-gray-400 hover:text-white transition-colors border-b border-white/10 pb-2 hover:border-white"
                   >
                     {social.name}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-white/10 text-center md:text-left gap-4">
           <p className="text-gray-500 font-medium text-sm">
             © {currentYear} Osama Adel. All rights reserved.
           </p>
           <p className="text-gray-500 font-medium text-sm">
