          ...
          <div className="lg:col-span-5 flex flex-col justify-between h-full gap-12">
            <div>
              ...
              >
                 ...
                 <span className="text-xs font-bold uppercase tracking-widest text-white/80">{t.contact.openToOpp}</span>
              </motion.div>
              ...
              >
                {t.contact.letsBuild} <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-purple-300 to-white">{t.contact.theFuture}</span>
              </motion.h2>
            ...
            <div className="grid gap-4">
               ...
               <GlassInfoCard 
                 ...
                 delay={0.4}
               />
               <GlassLocationCard delay={0.5} time={time} baseLabel={t.contact.base} timeLabel={t.contact.time} />
            </div>
          </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8 relative z-10">
                   ...
                   <div className="relative group">
                      <label 
                        htmlFor="message" 
                        className={cn(
                          "block text-xs font-bold uppercase tracking-widest mb-3 transition-colors duration-300 ml-1 rtl:mr-1 rtl:ml-0",
                   ...
                   <div className="flex flex-col-reverse md:flex-row justify-between items-center pt-4 gap-4">
                     <p className="text-xs md:text-sm text-gray-500 max-w-xs text-center md:text-left rtl:md:text-right">
                     ...
                     <div className="w-full md:w-auto">
                        <MagneticButton strength={50}>
                          ...
                          >
                            {formState === 'idle' && (
                              <>
                                <span className="z-10 relative">{t.contact.form.send}</span>
                                <Send className="w-5 h-5 z-10 relative group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform rtl:group-hover:-translate-x-1" />
                            ...
                            {formState === 'success' && (
                              <>
                                ...
                                <span className="text-sm">{t.contact.form.sent}</span>
                              </>
                            )}
                          </button>
                        </MagneticButton>
...
const GlassInfoCard = ({ icon: Icon, label, value, href, delay }: any) => (
  ...
  >
    ...
    <ArrowUpRight className="w-4 h-4 text-gray-500 ml-auto group-hover:text-white group-hover:translate-x-1 group-hover:-translate-y-1 transition-all flex-shrink-0 rtl:-rotate-90 rtl:mr-auto rtl:ml-0" />
  </motion.a>
);

const GlassLocationCard = ({ delay, time, baseLabel, timeLabel }: any) => (
  <motion.div 
    initial={{ opacity: 0, x: -10 }}
    whileInView={{ opacity: 1, x: 0 }}
    transition={{ delay, duration: 0.5 }}
  ...
  >
     <div className="flex items-center gap-3 md:gap-4">
        ...
        <div>
           <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest mb-0.5">{baseLabel}</p>
           <p className="text-base md:text-lg font-medium text-white">Istanbul, TR</p>
        </div>
     </div>
     <div className="text-right rtl:text-left pl-3 rtl:pl-0 rtl:pr-3 md:pl-4 rtl:md:pr-4 border-l rtl:border-l-0 rtl:border-r border-white/10 flex-shrink-0">
        <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest mb-0.5">{timeLabel}</p>
